...
class ConversationDataBuilder(ConcurrentGenerationDataBuilder):
    ...
    def _run_single_conversation(
        ...
        state outside `context`. seed_data is read-only; fetched once per
        ...
        list of live contexts, may append steps, discard contexts, or fork
        contexts into multiple (e.g., for DPO branching or rollout), and
        ...
        Args:
            context: Blank ConversationDataPoint created by task.get_batch_examples().
        ...
        Returns:
            ...
            starting context. Empty list if all contexts were discarded or
            min_turns was not reached on any surviving context.
        """
        # Phase 1: initialization stages (run once per conversation).
        # All contexts in the list are threaded through each stage together.
        contexts = [context]
        for stage in task.initialization_stages:
            contexts = stage(contexts, seed_data=seed_data)
            if not contexts:
                return []

        # Phase 2: iteration loop.
        # `live` holds contexts still accumulating turns.
        # `completed` accumulates contexts that finished with turn_count >=
        # min_turns (terminated by flow_controller or exhausted at max_turns).
        # Contexts dropped by a stage or terminated before min_turns are
        # silently discarded.
        ...
        while live:
            # Snapshot each live context at turn entry (Section 13.4).
            # If a context is dropped mid-turn and turn_count >= min_turns,
            # we rescue from the snapshot (end of turn N-1) rather than the
            # partial mid-turn state. This guarantees rescued contexts always
            # end at a complete turn boundary, which downstream SFT/DPO
            # serializers require.
            snapshots = {id(ctx): copy.deepcopy(ctx) for ctx in live}

            for stage in task.iteration_stages:
                previous = live
                live = stage(live, seed_data=seed_data)

                # Contexts the stage chose not to continue (omitted from
                # return). Rescue from the turn-entry snapshot so the saved
                # context ends at the last complete turn, not mid-turn.
                live_ids = {id(ctx) for ctx in live}
                for ctx in previous:
                    if id(ctx) not in live_ids:
                        if turn_count >= task.min_turns:
                            completed.append(snapshots[id(ctx)])
                            self.logger.info(
                                "[%s] conversation %s dropped mid-turn at turn %d/%d — rescued at turn %d",
                                task.name,
                                ctx.conversation_id[:8],
                                turn_count,
                                task.max_turns,
                                turn_count - 1,
                                extra={
                ...
                # Drain flow_controller-terminated contexts out of the live
                # pool. Only FlowControllerStep sets terminate=True; no other
                # stage uses this signal. Terminated contexts completed this
    ...
    def __call__(
        ...
        for task_name in {ctx.task_name for ctx in instruction_data}:
        ...
        with ThreadPoolExecutor(max_workers=inner_workers) as executor:
            # Each future gets its own copy_context() snapshot. A single Context
            ...
            futures = [
                executor.submit(
                    ...
                    ctx,
                    task_map[ctx.task_name],
                    seed_data_map[ctx.task_name],
                )
                for ctx in instruction_data
            ]
            return [ctx for f in futures for ctx in f.result()]
